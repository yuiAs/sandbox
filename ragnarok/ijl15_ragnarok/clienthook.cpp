...
HANDLE open_grf(LPCSTR a, DWORD b, DWORD c, LPSECURITY_ATTRIBUTES d, DWORD e, DWORD f, HANDLE g)
{
	...
	{
		HMODULE m = ::LoadLibrary(_T("COMDLG32.dll"));
		void* pfn_cd32_gofn = ::GetProcAddress(m, "GetOpenFileNameA");

		typedef BOOL (WINAPI *CD32_GOFN)(LPOPENFILENAME);
		if (reinterpret_cast<CD32_GOFN>(pfn_cd32_gofn)(&ofn) == FALSE)
			strcpy(filename, a);

		::FreeLibrary(m);
	}

	return reinterpret_cast<K32_CFILE_A>(pfn_k32_cfile)(filename, b, c, d, e, f, g);
}
...
HANDLE WINAPI k32_cfile(LPCSTR a, DWORD b, DWORD c, LPSECURITY_ATTRIBUTES d, DWORD e, DWORD f, HANDLE g)
{
	...
	if (ask_loading)
	{
		// sdata.grf
		// 012345678 = 9
		size_t ext = strlen(a) - 4;
		if (memcmp(a+ext, ".grf", 4) == 0)
...
#ifdef _UNICODE
	void* pfn_g32_cfi = ::GetProcAddress(::GetModuleHandle(_T("GDI32.dll")), "CreateFontIndirectW");
#else
	void* pfn_g32_cfi = ::GetProcAddress(::GetModuleHandle(_T("GDI32.dll")), "CreateFontIndirectA");
#endif	// #ifdef _UNICODE

	typedef HFONT (WINAPI *G32_CFI)(const LOGFONT*);
	return reinterpret_cast<G32_CFI>(pfn_g32_cfi)(&lf);
...
DWORD WINAPI imm_getclw(HIMC a, DWORD b, LPCANDIDATELIST c, DWORD d)
{
	...
	{
		const wchar_t* ws = reinterpret_cast<wchar_t*>(reinterpret_cast<char*>(c)+c->dwOffset[i]);
...
void hook::start()
{
	...
	ask_loading = conf->get_bool("filesystem", "ask_loading_grf");
	if (ask_loading || conf->get_bool("filesystem", "support_unicode"))
